...
(function($) {
	...
	Filestyle.prototype = {
		...
		constructor : function() {
			...
			//if (_self.options.btnMofi) {
			//	btn = '<label for="' + id + '"><button class="group-span-filestyle input-group-btn btn ' + _self.options.btnClass + '" '+
			//	  (_self.options.disabled || _self.$element.attr('disabled') ? ' disabled="true"' : '') +
			//	  ' type="button">' + _self.options.text + '</button></label>';
			//
			...
			//		  _self.$elementFilestyle.find('button').click();
			//			  return false;
			//		  }
			//	  });
			//}
			//else {		}		
			  ...
			  btn = '<span class="input-group-text group-span-filestyle ' + (_self.options.input ? 'input-group-btn' : '') + ' btn ' + _self.options.btnClass + '">' + 
				'<label for="' + id + '" style="margin-bottom: 0;" class=" ' +
