(function ($) {
  ...
  var _dataTable = _$salesTable.DataTable({
    ...
    language: {
      ...
      info: "Hiển thị _START_ đến _END_ trong _TOTAL_ sale",
      infoEmpty: "Hiển thị 0 đến 0 của 0 sale",
      infoFiltered: "(lọc từ _MAX_ tổng số sale)",
      lengthMenu: "Hiển thị _MENU_ sale",
  ...
  $(document).on('click', '.toggle-active-sale', function () {
    ...
    abp.message.confirm(
      'Bạn có chắc chắn muốn thay đổi trạng thái kích hoạt của sale này?',
      'Xác nhận',
      function (isConfirmed) {
        if (isConfirmed) {
          ...
          $.ajax({
            url: abp.appPath + 'Sales/ToggleActive',
            type: 'POST',
            data: JSON.stringify({ id: saleId }),
            contentType: 'application/json',
            success: function (result) {
              abp.notify.success('Đã thay đổi trạng thái thành công');
              _dataTable.ajax.reload();
            },
            error: function (error) {
              abp.notify.error('Có lỗi xảy ra: ' + (error.message || 'Không xác định'));
            },
            complete: function () {
              abp.ui.clearBusy();
            }
  ...
  function deleteSale(saleId, saleName) {
    abp.message.confirm(
      'Bạn có chắc chắn muốn xóa sale "' + saleName + '"?<br><small class="text-danger">Hành động này không thể hoàn tác!</small>',
      'Xác nhận xóa',
      function (isConfirmed) {
        if (isConfirmed) {
          abp.ui.setBusy();
          ...
          _saleService.deleteSale(saleId)
            .done(function () {
              abp.notify.success('Xóa sale thành công!');
              _dataTable.ajax.reload();
            })
            .fail(function (error) {
              abp.notify.error(error.message || 'Có lỗi xảy ra khi xóa sale');
            })
            .always(function () {
              abp.ui.clearBusy();
            });
