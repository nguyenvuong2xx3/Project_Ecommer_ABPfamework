(function ($) {
  $(function () {
    ...
    $('#SaveAllSettingsButton').click(function () {
      ...
      userManagement.userPasswordSettings = {
        enableCheckingLastXPasswordWhenPasswordChange: $('#Setting_EnableCheckingLastXPasswordWhenPasswordChange').is(':checked'),        
    ...
    $("#btnGeoUnit").click(function () {
      var _geoUnitService = abp.services.app.geoUnit;
      _geoUnitService.buildGeoVN().done(function () {
        abp.notify.info(app.localize('Successfully'));
      });
    });

    $('#btnCatUnit').click(function () {
      var _catUnitService = abp.services.app.categoryUnit;
      _catUnitService.configHostDefaultCategories().done(function () {
    		abp.notify.info(app.localize('Successfully'));
    	});
    });

    $("#ConfigurationBtn").click(function () {
      var _widgetTemplateService = abp.services.app.widgetTemplates;
      var _cmsSettingsService = abp.services.app.cmsSettings;

      var promise = $.Deferred().resolve().promise();
      //if ($('#GeoUnit').is(':checked')) {
        //promise = promise.then(function () {
        //  return $.ajax({
        //    url: '/api/services/app/GeoUnit/BuildGeoVN',
        //    method: 'POST',
        //    cache: false,
        //    success: function (results) {
        //      abp.notify.info(abp.localization.localize("Cập nhật danh mục địa chỉ thành công"));
        //    },
        ...
        //});
      //}
      //if ($('#CatUnit').is(':checked')) {

      //}

      //if ($('#ConfigUrlGroup').is(':checked')) {

      //}

      //if ($('#ResetDefaultSetting').is(':checked')) {
      //  promise = promise.then(function () {
      //    return _cmsSettingsService.resetLayout(1)
      //      .done(function () {
      //        abp.notify.info(app.localize('ResetDefaultTenantSetting Successfully'));
      //      })
      //      .fail(function () {
      //        abp.notify.error(app.localize('Failed to reset default tenant setting'));
      //      });
      //  });
      //}

      if ($('#SyncWidgetTemplateJson').is(':checked')) {
        promise = promise.then(function () {
          return _widgetTemplateService.configHostHtmlWidgetTemplate()
            .done(function () {
              abp.notify.info(app.localize('Cấu hình mẫu widget thành công'));
            })
            .fail(function () {
              abp.notify.error(app.localize('Cấu hình mẫu widget thất bại'));
            });
        });
      }
    });
