(function () {
  app.modals.ImportOrganizationUnitsDataModal = function () {
    var _modalManager;
    var _$form = null;
    ...
    this.init = function (modalManager) {
      ...
      _$form = _modalManager.getModal().find('form[name=ImportOrganizationUnitForm]');
      ...
      _$form.validate({
        ...
        errorPlacement: function (error, element) {
          ...
          console.log(element.closest('.input-group').length)
    ...
    this.save = function () {
      ...
      $.ajax({
        url: abp.appPath + 'App/OrganizationUnits/ImportData',
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
        success: function (response) {
          if (response.success) {
            ...
            abp.event.trigger('app.updateAttendanceTimeModalSaved');
            $('#ImportFileDataUploadArea').empty();
            $('#ImportFileData').filestyle('clear');
            _modalManager.close(); // đóng modal nếu muốn
          } else {
            abp.notify.error(response.message || 'Nhập dữ liệu thất bại!');
          }
        },
        error: function (xhr) {
          try {
            if (xhr.responseText) {
              ...
              abp.notify.error(errorResponse.message || "Đã xảy ra lỗi không xác định.");
            } else {
              abp.notify.error("Không nhận được phản hồi lỗi từ server.");
            }
          } catch (e) {
            abp.notify.error("Lỗi không thể phân tích phản hồi từ server.");
            console.error("Lỗi JSON parse:", e, "Dữ liệu:", xhr.responseText);
          }
        }
