(function () {
  app.modals.SelectRolesAddMailModal = function () {
    var _modalManager;
    var _mailConfigurationService = abp.services.app.attendanceViewRequest;
    var selectedObjs = []; // Biến toàn cục lưu trữ tất cả đối tượng đã chọn từ mọi trang
    var targetRoleIdsFromConfig = []; // chứa các RoleId cần tích sẵn
    var _options = {
    ...
    function loadSelectedObjsFromStorage() {
      var saved = localStorage.getItem('selectedRoles');
      if (saved) {
    ...
    function saveSelectedObjsToStorage() {
      localStorage.setItem('selectedRoles', JSON.stringify(selectedObjs));
    }
    ...
    function updateCheckboxStates() {
      dataTable.rows().every(function () {
        ...
        var shouldCheck = isObjSelected(rowData) || targetRoleIdsFromConfig.includes(parseInt(rowData.value));

        if (shouldCheck) {
          checkbox.prop('checked', true);
          this.select(); // chọn cả hàng
        } else {
    ...
    this.init = function (modalManager) {
      ...
      var orgUnitId = _modalManager.getArgs().organizationUnitId;

      _mailConfigurationService.getAllMailConfiguration( organizationUnitId = orgUnitId)
        .done(function (result) {
          // Gán danh sách các role cần tích checkbox
          console.log(result)
          targetRoleIdsFromConfig = result.map(x => x.targetRoleId); // hoặc x.TargetRoleId nếu chữ hoa

          // Sau khi có danh sách, update lại checkbox (nếu bảng đã render)
          updateCheckboxStates();
        });

      // Tải dữ liệu đã chọn từ localStorage
