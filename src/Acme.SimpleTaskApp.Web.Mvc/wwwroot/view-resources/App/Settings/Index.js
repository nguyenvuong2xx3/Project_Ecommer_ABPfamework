(function () {
	$(function () {
		...
		$('#ApplicationLightLogoImage').change(function () {
			...
			if (this.files && this.files[0]) {
				...
				fr.onload = function (e) {
					...
					var _cropImageModal = app.modals.CropImageModal.create({
						title: app.localize('SelectAUser'),
						imgData: this.result,
					});

					_cropImageModal.open({ ratioWidth: 126, ratioHeight: 100 }, function (croppedImgData) {
						SaveLogoLightInternal(croppedImgData);
					});
			...
			//   _cropImageModal.open({}, function (selectedItem) {
		...
		$('#PickFromMediasBtn').click(function () {
			_insertMediaModal.open({},
				function (selectedItems) {
					...
					_cropImageModal.open({ c: selectedItems.publicUrl, ratioWidth: 126, ratioHeight: 100 }, function (croppedImgData) {
						SaveLogoLightInternal(croppedImgData);
					});
		...
		function SaveLogoLightInternal(croppedImgData) {
			var token = app.guid();

			var formData = new FormData();
			formData.append('ProfilePicture', croppedImgData);
			formData.append('FileToken', token);
			formData.append('FileName', "TenantLightLogo");

			$.ajax('/TenantCustomization/UploadLightLogoBlob', {
				method: "POST",
				data: formData,
				processData: false,
				contentType: false,
		...
		$('#Color').change(function () {
			...
			$.ajax({
				url: '/TenantCustomization/ChangeCss',
				type: 'POST',
				data: formData,
				processData: false,
				contentType: false,
		...
		$('#btnCatUnit').click(function () {
			var _catUnitService = abp.services.app.categoryUnit;
			_catUnitService.configTenantDefaultCategories().done(function () {
				abp.notify.info(app.localize('Successfully'));
			});
		});
		
		$('#btnConfigOrganizationUnit').click(function () {
			$.ajax({
				url: '/api/services/app/HrmSettings/ConfigOrganizationUnit',
				cache: false,
				type: "POST",
				success: function (results) {
					abp.notify.info(abp.localization.localize("Cập nhật cơ cấu tổ chức trường Đại học ngoại ngữ thành công"));
				},
		...
		function updateAppSettings() {
			abp.services.app.saaSAppSetting.updateAppSettings({
