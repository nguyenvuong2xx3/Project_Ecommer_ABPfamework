(function () {
    ...
    var _userExporterService = abp.services.app.userExporter;
		...
		var dataTable = _$usersTable.DataTable({
			//var dataTable = jQuery('#UsersTable').dataTable({
			paging: true,
			serverSide: true,
			processing: true,
			listAction: {
			...
			columnDefs: [
				{
					...
					rowAction: {
						...
						items: [
							...
							{
								text: app.localize('AddToHikPersons'),
								visible: function (data) {
									return _permissions.edit;
								},
								action: function (data) {
									abp.services.app.hikSync.syncUserToHikPersonal(
										data.record.id
									).done(function () {
										//getHikPersonRecords(true);
							...
							},
							{
								...
								visible: function () {
									return _dynamicEntityPropertyManager.canShow(
										'BBK.SaaS.Authorization.Users.User'
									);
								},
								action: function (data) {
									_dynamicEntityPropertyManager.modal.open({
										entityFullName: 'BBK.SaaS.Authorization.Users.User',
				...
				},
				{
					...
					render: function (isEmailConfirmed) {
						...
						var $badge = $('<div/>');
						if (isEmailConfirmed) {
							$badge.addClass('badge bg-success').text(app.localize('Yes'));
						} else {
							$badge.addClass('badge bg-light text-dark').text(app.localize('No'));
						}
      ...
      e.preventDefault();
      _userExporterService
        .exportDataHikCentral({
          filter: $('#UsersTableFilter').val(),
          permissions: _selectedPermissionNames,
          role: $('#RoleSelectionCombo').val(),
          onlyLockedUsers: $('#UsersTable_OnlyLockedUsers').is(':checked'),
